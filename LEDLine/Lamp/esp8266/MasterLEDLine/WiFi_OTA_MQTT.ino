...
#define MQTT_TOPIC_PUB1     MQTT_USERNAME"/master/state"
#define MQTT_TOPIC_PUB2     MQTT_USERNAME"/slave/effect"
...
// Setup MQTT topics
Adafruit_MQTT_Publish   mqttMasterState(&mqtt, MQTT_TOPIC_PUB1);
Adafruit_MQTT_Publish   mqttSlaveEffect(&mqtt, MQTT_TOPIC_PUB2);
...
void processMQTT()
{
	...
	if (f_publishState && mqtt.connected())
	{
		...
		if (!mqttMasterState.publish(currentEffect))
		{
...
void publishSlaveEffect()
{
	if (mqtt.connected())
	{
		...
		if (!mqttSlaveEffect.publish(currentEffect))
		{
			Serial.println(F("Publish Message Failed"));
			return;
		}
