...
#define MQTT_TOPIC_PUB1     MQTT_USERNAME"/slave/state"
#define MQTT_TOPIC_SUB1     MQTT_USERNAME"/set/effect"
#define MQTT_TOPIC_SUB2     MQTT_USERNAME"/set/action"
#define MQTT_TOPIC_SUB3     MQTT_USERNAME"/slave/effect"
...
// Setup MQTT topics
Adafruit_MQTT_Publish   mqttClientState(&mqtt, MQTT_TOPIC_PUB1);
...
Adafruit_MQTT_Subscribe mqttSlaveEffect(&mqtt, MQTT_TOPIC_SUB3, MQTT_QOS_1);
...
void setup_MQTT()
{
	mqttSlaveEffect.setCallback(mqtt_setEffect_callback);
	mqtt.subscribe(&mqttSlaveEffect);
...
void processMQTT()
{
	...
	if (f_publishState && mqtt.connected())
	{
		...
		if (!mqttClientState.publish(currentEffect))
		{
