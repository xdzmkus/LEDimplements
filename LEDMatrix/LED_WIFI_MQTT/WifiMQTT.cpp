...
WifiMQTTClass::WifiMQTTClass()
{
	...
	setRunningString.setCallback(setRunningString_callback);
	mqtt.subscribe(&setRunningString);
...
void WifiMQTTClass::log(LOG_LEVEL level, String msg)
{
	...
	Adafruit_MQTT_Publish garlandLog(&mqtt, log_topic.c_str());

	if (!keepAliveMQTT() || !garlandLog.publish(msg_keyword.c_str()))
	{
